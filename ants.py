            ...
            if insect.is_ant:
                if self.ant is None:
                    ...
                    if self.ant.can_contain(insect):
                        ...
                        insect.ant, self.ant = self.ant, insect
                    else:
...
class ThrowerAnt(Ant):
    ...
    food_cost = 3
    damage = 1
    min_range, max_range, distance = 0, float("inf"), 0
    ...
    def nearest_bee(self, hive):
        ...
        """
        bee_location, distance = self.place, 0
        while bee_location != hive :
            if len(bee_location.bees) != 0 and distance >= self.min_range and distance <= self.max_range:
                return random_or_none(bee_location.bees)
            distance += 1
            bee_location = bee_location.entrance
        return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            for bee in current_location.bees[:]:
                bee.armor -= self.damage
            current_location.bees = [bee for bee in current_location.bees if bee.armor > 0]
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        for bee in self.place.bees[:]:
            bee.armor -= self.damage
        self.place.bees = [bee for bee in self.place.bees if bee.armor > 0]


class ScubaThrower(ThrowerAnt):
    '''ScubaAnt is a ThrowerAnt that can be placed in the water'''
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        Ant.__init__(self)
        self.imposter = QueenAnt.impostor
    ...
    def action(self, colony):
        ...
        if not self.imposter:
            super().action(colony)
            current_location = self.place.exit
            while current_location != None:
                current_ant = current_location.ant
                if current_ant:
                    if current_ant not in self.buffed_ants:
                        current_ant.damage *= 2
                        self.buffed_ants.append(current_ant)
                    if current_ant.container and current_ant.ant != None and current_ant.ant not in self.buffed_ants:
                        current_ant.ant.damage *= 2
                        self.buffed_ants.append(current_ant.ant)
                current_location = current_location.exit
        else:
